...
Уместно использованные ссылки улучшают доступность интерфейса: по `Ctrl + клик` или средней кнопкой мыши можно легко открывать документ в новой вкладке; экранные читалки могут собирать все ссылки на странице в один список (чтобы пользователь не перебирал их раз за разом), и чем меньше в этом списке мусора с `<a href="#">` и т. п., тем быстрее получится отыскать интересующую информацию.
